...
def render_acoes(user: dict):
    ...
    st.html(
        ...
        + kpi_h("📊 Margem Bruta M&T",  _ed(total_margem),
                "COMPETE2030 + Faturação às Empresa − Custo Consultores - Custo Formadores",
    ...
    for a in acoes:
        ...
        st.html(
            ...
            # Margem
            f'<div style="padding:12px 14px;background:{mg_bg}"><div style="font-size:10px;color:{m_cor};font-weight:600;text-transform:uppercase;letter-spacing:.06em;margin-bottom:4px">Margem M&T</div><div style="font-size:11px;color:#8B94A3;margin-bottom:4px">COMPETE2030 + Faturação à Empresa − Custo Consultor - Custo Formador</div><div style="font-size:16px;font-weight:700;color:{m_cor}">{_ed(margem) if v_comp else "—"}</div></div>' +
